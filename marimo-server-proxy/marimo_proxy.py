...
def setup_marimoserver():
	...
	base_url = os.environ.get("JUPYTERHUB_SERVICE_PREFIX", "{base_url}")
	if not newpath:
	...
	return {
		'command': ['marimo', 'edit', '--port', '{port}', '--base-url', f'{base_url}marimo', '--token', '--token-password', token, '--headless'],
